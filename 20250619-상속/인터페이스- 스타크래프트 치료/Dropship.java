...
public class Dropship extends AirUnit implements Repair { // Dropship은 AirUnit(공중 유닛)의 자식이고, Repair 인터페이스를 구현한다!

	public Dropship() { // Dropship의 초기 체력은 고정값으로 설정.
		// ★★★ 중요! 부모 클래스 AirUnit의 생성자 호출! ★★★
		// Unit 생성자에서 hitPoint도 MAX_HP로 초기화해주므로 `hitPoint = MAX_HP;`는 필요 없다!
		super(125); // Dropship의 기본 체력은 125로 설정한다.
	}
	
	@Override
	public String toString() { 
		return "Dropship"; // Dropship 객체를 문자열로 나타낼 때 "Dropship"이라고 반환한다.
	}
