...
/*
  ...
  - 그리고 그 바깥쪽 `catch (Exception e)`도 같은 `main` 메소드 스코프 안에서 `e`라는 변수를 **또 다시 선언하려고 해서 컴파일 에러가 나는 거다 (이미 `e`가 있다고 지랄하는 거지!)
		...
		try { // 첫 번째 'try' 블록 시작
			try { // 안쪽 'try' 블록 시작
				...
				// 이 'e' 변수는 이 안쪽 'catch' 블록의 스코프 내에서만 유효하다.
                                 ...
                                 // 'main' 메소드라는 더 큰 동일 스코프 내에서 이 'e'를 다시 선언하려 하므로 **컴파일 에러가 발생한다!**
