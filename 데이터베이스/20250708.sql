...
-- 연봉을 계산하기 위해 급여에 12를 곱하고 커미션을 더함.
...
-- 2. 부서번호가 30번인 사원들의 이름, 직급, 부서번호, 부서위치를 출력하시오
SELECT E.ENAME, E.JOB, E.DEPTNO, D.LOC -- E.MGR 대신 E.JOB 추가
FROM EMP E, DEPT D
WHERE E.DEPTNO = D.DEPTNO AND E.DEPTNO = 30; -- 조인 조건 추가 및 30번 부서 필터링

-- 3. 커미션을 받는 사원의 이름, 커미션, 부서 이름 및 부서 위치 출력하시오
SELECT E.ENAME, E.COMM, D.DNAME, D.LOC -- D.LOC 추가
FROM EMP E, DEPT D
WHERE E.DEPTNO = D.DEPTNO AND E.COMM IS NOT NULL; -- 조인 조건 추가 및 커미션 조건 추가

-- 4. DALLAS에 근무하는 사원에 이름, 직급, 부서번호, 부서이름을 출력하시오
SELECT E.ENAME, E.JOB, E.DEPTNO, D.DNAME
FROM EMP E, DEPT D
WHERE E.DEPTNO = D.DEPTNO AND D.LOC = 'DALLAS'; -- 조인 조건 추가 및 'DALAS' -> 'DALLAS' 오타 수정

-- 5. 이름에 A가 들어가는 사원들의 이름과 부서 이름을 출력하시오
SELECT E.ENAME, D.DNAME
FROM EMP E, DEPT D
WHERE E.DEPTNO = D.DEPTNO AND E.ENAME LIKE '%A%'; -- 조인 조건 추가
...
-- 7. 사원 이름, 부서이름, 부서번호와 해당 사원과 같은 부서에서 근무하는 사람을 출력하시오. (SELF JOIN)
SELECT E1.ENAME AS 사원, D.DNAME AS 부서이름, D.DEPTNO AS 부서번호, E2.ENAME AS 같은_부서_동료
FROM EMP E1
