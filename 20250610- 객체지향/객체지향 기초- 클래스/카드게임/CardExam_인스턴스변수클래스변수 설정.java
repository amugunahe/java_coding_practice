...
class Card {
	...
	// 'Card' 객체를 만들 때마다 각각 생성되며, 각 객체마다 다른 값을 가질 수 있다.
	String kind;   // 카드의 무늬 (예: "Heart", "Spade")
	int number;    // 카드의 숫자 (예: 7, 4)

	// --- 2. 클래스 변수 (Class variable, static 변수) ---
	// 'static'이 붙어있으며, 클래스가 메모리에 로딩될 때 딱 한 번 생성된다.
	// 모든 'Card' 객체가 이 변수들을 ★공유★한다.
	static int width = 100;    // 카드의 기본 너비 (모든 카드가 공유)
	static int height = 250;   // 카드의 기본 높이 (모든 카드가 공유)
}
