...
class Div {
	...
	// 3. 나눗셈 계산 메서드
	public int calculate() { // 나눗셈 결과 반환.
		// 0으로 나누는 건 'Calc'에서 이미 막는다.
		return a / b; // 'a'를 'b'로 나눈 값 (정수 나눗셈이라 소수점 버림).
	}
}
...
public class Calc { // 'Calc' 클래스 정의.
    ...
    public static void main(String[] args) { // 프로그램 시작점! 여기서부터 실행된다.
        ...
        case '/':
            if (b == 0) { // 만약 두 번째 숫자가 0이면 (0으로 나누면 프로그램 터짐!)
                System.out.println("오류: 0으로 나눌 수 없습니다."); // 에러 메시지 출력.
            } else {
