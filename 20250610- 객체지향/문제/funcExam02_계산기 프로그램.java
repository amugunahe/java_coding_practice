...
/*
  funcExam02 클래스: **사용자한테 메뉴 번호를 입력받아 여러 가지 계산 기능을 수행하는 존나 유용한 콘솔 계산기다!**
  * 핵심:
  - **목표:** '+' (더하기), '-' (빼기), '*' (곱하기), '/' (나누기), '%' (나머지) 연산을
  각각 별도의 **메소드(method)**로 구현해서 프로그램의 재사용성과 가독성을 존나게 높였다!
  - **메소드 분리:** `plus()`, `minus()`, `multi()`, `divi()`, `per()`는 각 연산을 담당하고,
  `disp()`는 계산 결과를 깔끔하게 출력하는 역할을 한다.
  - **`BufferedReader`:** 사용자 입력을 한 줄씩 읽어올 때 쓰는 존나 유용한 놈이다!
...
public class funcExam02 { // 'funcExam02'라는 이름의 공개(public) 클래스를 선언한다.
